...
// (see index.html) is ready. Guarded everywhere below by `window.cast &&
// cast.framework`, since browsers with no Cast support (e.g. Firefox) never
// call this at all and castMedia()/the Cast button must degrade gracefully.
...
async function castMedia(item, streamUrl, hlsPlaylistUrl) {
  ...
  try {
    ...
    // Deliberately announce() rather than showPlayerMessage() -- the latter
    // wipes #player-container, which would kill whatever's still playing
