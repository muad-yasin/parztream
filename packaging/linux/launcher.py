"""Entry point for the Linux AppImage build (see parztream.spec / AppRun).

Not app/main.py directly -- mirrors packaging/windows/launcher.py, solving
the same class of problems that only exist once parztream is a
self-contained, double-clickable/executable bundle instead of something
started from a terminal in the project checkout:

- A PyInstaller onefile build unpacks itself into a temporary folder
  (sys._MEIPASS) that's deleted after every run -- an AppImage on top of
  that runs from an even more temporary FUSE mount. app/config.py's own
  ...
  (following the XDG Base Directory spec: $XDG_DATA_HOME, or
  ~/.local/share, falling back to the home directory) before
  app/config.py ever gets imported and reads those environment variables
  at import time.
...
- Opens the default browser automatically once the server is actually
  ready to accept connections. This is expected to silently do nothing
  on a headless Linux server (no display, no browser) -- a very real
  way people run this specific app -- the startup banner prints the URL
  to open from another device instead.
"""
...
def _persistent_secret_key(data_dir: Path) -> str:
    ...
    try:
        ...
        # A no-op on filesystems/platforms without POSIX permission bits
        # (e.g. Windows' os.chmod only toggles the read-only bit).
...
def _open_browser_once_ready(url: str) -> None:
    def wait_and_open():
        ...
        try:
            ...
            # No browser available (e.g. a headless server) -- the startup
            # banner already printed the URL, so this is silently
            # skippable rather than a real failure.
            pass
...
def main() -> None:
    ...
    print("  To stop parztream, press Ctrl+C (or close this window).")
    print("=" * 64)
