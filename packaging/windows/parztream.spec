...
#
# Builds the onefile Windows executable. Run from the repository root:
#   pyinstaller packaging/windows/parztream.spec --distpath dist
#
# packaging/windows/vendor/ffmpeg/{ffmpeg.exe,ffprobe.exe} are bundled in if
# present (see .github/workflows/build-windows-exe.yml, which downloads them
# before this runs) -- the build still works without them, it just produces
# an exe that behaves like running parztream with no ffmpeg on PATH (see
# README's "What you'll need" for what that means).

from pathlib import Path

REPO_ROOT = Path(SPECPATH).resolve().parent.parent
FFMPEG_DIR = REPO_ROOT / "packaging" / "windows" / "vendor" / "ffmpeg"
